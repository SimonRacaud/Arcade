...
SFML::SFML()
    : _isMouseClicked(false), _isOpen(false), _scaleX(SCALE_X),
      _scaleY(SCALE_Y), _textSize(TEXT_SIZE)
{
...
void SFML::putRectFill(Color color, arcade::Coord size, arcade::Coord pos)
{
    ...
    rectangle.setSize(
        sf::Vector2f((float) size.x * _scaleX, (float) size.y * _scaleY));
    rectangle.setPosition(
        sf::Vector2f((float) pos.x * _scaleX, (float) pos.y * _scaleY));
...
void SFML::putCircle(Color color, Coord pos, size_t radius)
{
    ...
    circle.setPosition(
        sf::Vector2f((float) pos.x * _scaleX, (float) pos.y * _scaleX));
