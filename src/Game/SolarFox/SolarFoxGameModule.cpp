...
void SolarFoxGameModule::generateCoin()
{
    ...
    for (size_t i = 0; i < pattern.size(); i++) {
        ...
        this->_coins.back()->setPosition(pattern[i]);
        this->_coins.back()->setAnimation(Color::GREEN, 5);
    }
}
...
{
    if (_difficulty == Difficulty::EASY) {
        this->_enemies.push_back(new SolarFoxEnemy(_map.getSize(), {1, 1}, SolarFoxEnemy::Direction::RIGHT));
        this->_enemies.back()->setPosition({1, 1});
    }
    if (_difficulty == Difficulty::MEDIUM) {
        this->_enemies.push_back(new SolarFoxEnemy(_map.getSize(), {1, 38}, SolarFoxEnemy::Direction::RIGHT));
        this->_enemies.back()->setPosition({1, 38});
    }
    if (_difficulty == Difficulty::HARD && _enemies.size() != 4) {
        this->_enemies.push_back(new SolarFoxEnemy(_map.getSize(), {1, 1}, SolarFoxEnemy::Direction::DOWN));
        this->_enemies.back()->setPosition({1, 1});
        this->_enemies.push_back(new SolarFoxEnemy(_map.getSize(), {38, 38}, SolarFoxEnemy::Direction::UP));
        this->_enemies.back()->setPosition({38, 38});
    }
}